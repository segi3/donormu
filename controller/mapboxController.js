...
const _fetchMapboxRoute = async (start, end) => {
    try {
        const response = await axios.get(url + 'directions/v5/mapbox/walking/' + start.long + ',' + start.lat + ';' + end.long + ',' + end.lat + '?overview=full&geometries=geojson&access_token=' + process.env.MAP_BOX_KEY)
